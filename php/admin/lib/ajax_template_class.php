...
if($action=="class_add"){
	...
	$rss= $db->fetch_first("select * from {$tablepre}template_class where title='$title'  limit 1");
	if(!$rss){
		$sql="insert into {$tablepre}template_class(title,sid,dateline,fpic) value ('$title',$sid,'$timestamp','$fpic')";
		$query = $db->query($sql);
		echo'{"state":1,"msg":"OK"}';
	}else{
...
if($action=="class_edit"){
	...
	$query = $db->query("select * from {$tablepre}template_class where title='$title' and tid<>$tid ");
	...
	$rss= $db->fetch_first("select * from {$tablepre}template_class where tid=$tid  limit 1");
	if($rss){
		$tid=$rss["tid"];
		$sql="update {$tablepre}template_class set title='$title',fpic='$fpic',sid=$sid where tid=$tid";
		$query = $db->query($sql);
		echo'{"state":1,"msg":"OK"}';
	}else{
...
if($action=="class_dell"){
	...
	if($rss){
		...
		$db->query("delete from {$tablepre}template_class where tid=$tid");
		echo'{"state":1,"msg":"OK"}';
	}else{
		echo'{"state":0,"msg":"参数错误"}';
	}
